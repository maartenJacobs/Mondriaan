...
public:
  ...
  /**
   * \brief Determine the position of the codel that should be entered from the
   * exit position. \return nullptr if the next position starting from the exit
   * position is an invalid point. Otherwise the next position from the exit
   * position, belonging to the codel block that should be entered.
   */
...
struct CodelBlock {
  ...
  ExitPosition *topLeftExit = nullptr, *topRightExit = nullptr,
...
/**
 * @brief Piet::WhiteBlockParser parses the transition of codels across a white
 * block.
 * @paragraph Starting from the exit point's direction, we'll "slide" across the
 * white block to the next non-black codel. The rules for this transition are
 * different than the regular transition rules of the Piet language. For white
 * codels we move straight onwards instead of finding the furthest point in the
 * direction of the Direction Pointer and the Codel Chooser. Additionally, the
 * Codel Chooser and the Direction Pointer are both flipped when a black codel
 * or an edge is reached (the CC is flipped twice to flip the DP). Similar to
 * the regular rules, we flip the DP 4 times (similar to the 8 tries of the
 * regular rules) before giving up.
 * @paragraph This algorithm gives us either the next codel (non-black and
 * non-white) or the white codel, in which case the white codel should be
 * considered terminal.
 * @paragraph Note that the Piet language spec mentions "If [the interpreter]
 * retraces its route entirely within a white block, there is no way out of the
 * white block and execution should terminate." The npiet interpreter does not
 * implement this rule; instead programs consisting of 1 coloured codel in the
 * top-left corner with a white codel around and nothing else result in an
 * infinite loop. The Mondriaan compiler will follow the specification instead.
 */
...
public:
  ...
  GraphEdge *parse(Position startPosition, DirectionPoint startDirection,
      ...
      while ((nextPosition = move(currentDirection, image, currentPosition)) !=
        ...
        if (currentColor == Black) {
          ...
          return new GraphEdge{new DirectionPoint{currentDirection},
                               colouredBlockOwner->constructingNode, true};
        }
      }
    ...
    // We were unable to find another coloured codel. The white block will serve
    // as a terminal block then.
    auto whiteBlockOwner = blocks.at(image->ownerAt(startPosition) - 1);
    return new GraphEdge{new DirectionPoint{currentDirection},
                         whiteBlockOwner->constructingNode, true};
  }
...
GraphEdge *edgeFromExitPosition(Image *image,
                                WhiteBlockParser *whiteBlockParser,
                                const vector<CodelBlock *> &blocks,
                                ExitPosition *exit) {
...
Graph *Parser::parse() {
  ...
  while (!visitPositions.empty()) {
    ...
    block->constructingNode = new GraphNode(
        block->color, block->size, identifierFromParts(identifierParts));
  ...
  for (auto block : blocks) {
    ...
    if (block->rightTopExit) {
      block->constructingNode->connect(
          ...
          edgeFromExitPosition(image, whiteBlockParser, blocks,
                               block->rightTopExit));
    }
    if (block->rightBottomExit) {
      block->constructingNode->connect(
    ...
    bool isTerminalNode =
        block->constructingNode->getColor() == White ||
