...
namespace Piet {
    ...
    void Translator::registerPietGlobals() {
        ...
        FunctionType *pushType = FunctionType::get(Type::getVoidTy(context), pushArgs, false);
        ...
        // Register pointer.
        FunctionType *pointerType = FunctionType::get(Type::getInt32Ty(context), vector<Type *>(), false);
        pointerBranch = Function::Create(pointerType, Function::ExternalLinkage, "mondriaan_runtime_pointer", &module);

        // Register in(number);
        FunctionType *inNumberType = FunctionType::get(Type::getVoidTy(context), vector<Type *>(), false);
    ...
    Function *Translator::translateBranch(Parse::GraphNode *node, DirectionPoint dp) {
        graph->restartWalk(node, dp);

        Function *openFunction = Function::Create(
                FunctionType::get(Type::getVoidTy(context), false),
                Function::PrivateLinkage,
                "next_mondriaan_sequence",
                &module);

        BasicBlock *openBlock = BasicBlock::Create(context, "mondriaan_seq", openFunction);
        ...
        while ((step = graph->walk()) != nullptr) {
            ...
            cout << "previous color: " << hex << step->previous->getColor() << dec << "; previous size: " << step->previous->getSize() << endl;
            ...
            if (transition == nullptr) {
                ...
                if (operation == OP_PUSH) {
                    ...
                    BasicBlock *noTurnJumpBlock = BasicBlock::Create(context, "mondriaan.pointer.jmp.0", openFunction);
                    builder.SetInsertPoint(noTurnJumpBlock);
                    builder.CreateCall(noTurnBranch);
                    builder.CreateRetVoid();
                    BasicBlock *singleTurnJumpBlock = BasicBlock::Create(context, "mondriaan.pointer.jmp.1", openFunction);
                    builder.SetInsertPoint(singleTurnJumpBlock);
                    builder.CreateCall(singleTurnBranch);
                    builder.CreateRetVoid();
                    BasicBlock *doubleTurnJumpBlock = BasicBlock::Create(context, "mondriaan.pointer.jmp.2", openFunction);
                    builder.SetInsertPoint(doubleTurnJumpBlock);
                    builder.CreateCall(doubleTurnBranch);
                    builder.CreateRetVoid();
                    BasicBlock *tripleTurnJumpBlock = BasicBlock::Create(context, "mondriaan.pointer.jmp.3", openFunction);
                    builder.SetInsertPoint(tripleTurnJumpBlock);
                    builder.CreateCall(tripleTurnBranch);
                    builder.CreateRetVoid();

                    // Create 3 blocks for the 4-way logic.
                    BasicBlock *doubleTurnTestBlock = BasicBlock::Create(context, "mondriaan.pointer.test.2", openFunction);
                    builder.SetInsertPoint(doubleTurnTestBlock);
                    const auto twoVal = builder.getInt(APInt(32, 2));
                    Value *twoCondition = builder.CreateICmp(CmpInst::ICMP_EQ, pointerVal, twoVal);
                    builder.CreateCondBr(twoCondition, doubleTurnJumpBlock, tripleTurnJumpBlock);
                    BasicBlock *singleTurnTestBlock = BasicBlock::Create(context, "mondriaan.pointer.test.1", openFunction);
                    builder.SetInsertPoint(singleTurnTestBlock);
                    const auto oneVal = builder.getInt(APInt(32, 1));
                    Value *oneCondition = builder.CreateICmp(CmpInst::ICMP_EQ, pointerVal, oneVal);
                    builder.CreateCondBr(oneCondition, singleTurnJumpBlock, doubleTurnTestBlock);
                    BasicBlock *noTurnTestBlock = BasicBlock::Create(context, "mondriaan.pointer.test.0", openFunction);
                    builder.SetInsertPoint(noTurnTestBlock);
                    const auto zeroVal = builder.getInt(APInt(32, 0));
                    Value *zeroCondition = builder.CreateICmp(CmpInst::ICMP_EQ, pointerVal, zeroVal);
                    builder.CreateCondBr(zeroCondition, noTurnJumpBlock, singleTurnTestBlock);
                     ...
                     *  br i1 %cond0, label %mondriaan.pointer.jmp.0, label %mondriaan.pointer.test.1
                     *
                     * mondriaan.pointer.test.1:
                     *  %cond1 = icmp eq i32 %pointer, %1
                     *  br i1 %cond1, label %mondriaan.pointer.jmp.1, label %mondriaan.pointer.test.2
                     *
                     * mondriaan.pointer.test.2:
                     *  %cond2 = icmp eq i32 %pointer, %2
                     *  br i1 %cond2, label %mondriaan.pointer.jmp.2, label %mondriaan.pointer.jmp.3
                     *
    ...
    void Translator::translateToExecutable(string filename) {
        ...
        auto mainFunction = cast<Function>(module.getOrInsertFunction("main",
                                                                        ...
                                                                        IntegerType::getInt8Ty(context)));
        {
            ...
            BasicBlock *entryBlock = BasicBlock::Create(context, "main_seq", mainFunction);
